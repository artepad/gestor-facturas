...
**Respaldo y restauración** (`src/respaldo.py`): empaqueta en un `.zip` la BD (`sqlite3.backup()` para copia atómica), los PDFs de `rutas.archivo` (incluye `_revisar`/`_no_facturas`/`_reemplazadas`/`_errores`; **excluye** `_entrada` y los logs), `config.yaml` y opcionalmente `.env`. El `manifiesto.json` lleva `tipo`, `version_esquema_respaldo`, `version_esquema_bd`, `version_programa`, conteos (`facturas`/`pdfs`/`alias`) y hashes SHA-256 de los archivos clave; `verificar_zip` los re-calcula al abrir y rechaza el respaldo si no coinciden. `importar` usa el `config_local` del PC destino (no el del zip) para decidir dónde poner BD y PDFs, así un respaldo hecho en `C:\Facturas` se restaura sin cambios en otro PC que tenga la raíz en `D:\Facturas`. **Coordinación cross-process**: `bloquear_procesamiento(config)` crea un archivo marcador `.respaldo_en_progreso` junto a la BD; `procesar()` en `main.py` (el watcher) lo respeta y deja los PDFs en `_entrada` hasta que el bloqueo se libere (auto-recovery si el marcador queda huérfano más de 1h). **Red de seguridad**: al importar, el `DialogoImportarRespaldo` genera primero un respaldo automático en `data/respaldos_automaticos/respaldo_AUTO_antes_de_importar_*.zip` (`limpiar_respaldos_automaticos` mantiene solo los 5 más recientes) — si la importación sale mal o el usuario se arrepiente, recupera desde ese mismo zip. La UI vive en `buscador.py` como `DialogoExportarRespaldo`/`DialogoImportarRespaldo` (modales con `ttk.Progressbar`, trabajo en thread + `queue.Queue` consumida por `after()`), accesibles desde el botón **"Respaldo ▾"** en la barra inferior del Administrador.
