...
**Base de datos** (`db.py`): SQLite con tablas `facturas`, `alias_proveedor`, `empresa_rut`, `instruccion_proveedor`, `producto`, `detalle_factura` y una tabla FTS5 (`facturas_fts`) mantenida por triggers para búsqueda full-text. `PRAGMA foreign_keys` está activo en cada conexión: borrar una factura arrastra en cascada su detalle de productos. El buscador Tkinter (`buscador.py`) consulta por texto libre, proveedor y rango de fechas; doble clic en un resultado abre `ventana_factura.py`, una ventana con el PDF embebido (renderizado con PyMuPDF) a la izquierda y los datos + la tabla de productos a la derecha.

**Extracción de detalle (bajo demanda, fuera del pipeline automático)**: la ventana de factura tiene un botón "Analizar productos con IA" que llama a `extractor.extraer_completo` (texto + imágenes de todas las páginas; una página muy alta se parte en franjas para no perder resolución) y `classifier.Clasificador.extraer_detalle` (herramienta forzada `registrar_detalle_factura`, acepta `instrucciones` opcionales del usuario para facturas de formato complejo). El resultado —productos línea por línea + si los precios traen IVA— se guarda en `producto`/`detalle_factura` y queda cacheado: reabrir la factura muestra lo guardado sin volver a llamar la API. El análisis corre en un hilo aparte para no congelar la ventana. Esto NO ocurre en el escaneo automático, solo cuando el usuario lo pide.

**Memoria de instrucciones por proveedor**: cuando el usuario re-analiza una factura y le da instrucciones a la IA sobre cómo interpretar ese formato, las instrucciones se guardan en `instruccion_proveedor` con la clave del RUT del emisor (`db.guardar_instrucciones`). En el primer análisis de cualquier factura futura del mismo RUT, esas instrucciones se inyectan automáticamente (`db.obtener_instrucciones`), así el sistema "aprende" a leer las facturas de cada proveedor y no hay que corregir lo mismo cada vez.

**Precio de venta sugerido**: `precios.py` calcula el precio sugerido de cada producto (valor unitario = `monto/cantidad` → + IVA si el producto es afecto y el precio venía neto → + margen → redondeo hacia arriba). Los parámetros (`iva`, `margen`, `redondear_a`) viven en `config.yaml` bajo `precios`. En la tabla de la ventana, doble clic en una celda corrige el valor a mano (recalcula el precio y marca la fila como editada); el detalle se recalcula vía `db.recalcular_precios`.
